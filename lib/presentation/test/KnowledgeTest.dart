...
class MyKnowledge extends StatefulWidget {
  const MyKnowledge({Key? key}) : super(key: key);

  @override
  State<MyKnowledge> createState() => _MyKnowledgeState();
}
...
class Knowledge {
  ...
  Knowledge({required this.id, required this.Name,
...
List<Knowledge> itemsofSearch = [];
DatabaseReference ref = LanguageGlobal.isEnglish? FirebaseDatabase.instance.ref("Knowledge")
    : FirebaseDatabase.instance.ref("KnowledgeArabic");

class _MyKnowledgeState extends State<MyKnowledge> {
  List<Knowledge> _knowledge = [];
  List<Knowledge> _knowledge2 = [];
  ...
  Future<void> _loadDataLocally() async {
    ...
    setState(() {
      _knowledge = data.map((user) => Knowledge.fromMap(user)).toList();
      _knowledge2= data.map((user) => Knowledge.fromMap(user)).toList();
    });
  }
                                    ...
                                    GestureDetector(
                                      onTap: (){
                                        Navigator.of(context)
                                            ...
                                            MyKnowledgeDetails(detailsModel: items[index],)));
                                      },
                                      child: Container(
